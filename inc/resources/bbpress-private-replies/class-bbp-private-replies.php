<?php
class Hovard_BBP_Private_Replies {
	...
	public function checkbox() {
        ...
        <p>
			...
			<?php if ( bbp_is_reply_edit() && ( get_the_author_meta( 'ID' ) != bbp_get_current_user_id() ) ) : ?>
                ...
                <label for="bbp_private_reply"><?php esc_html_e( "Set author's post as private.", 'disputo' ); ?></label>

			<?php else : ?>

                <label for="bbp_private_reply"><?php esc_html_e( 'Set as private reply', 'disputo' ); ?></label>
	...
	public function hide_reply( $content = '', $reply_id = 0 ) {
		...
		if( $this->is_private( $reply_id ) ) {
			...
			if( ! $can_view ) {
				$content = '<div class="alert alert-danger">' . esc_html__( 'This reply has been marked as private.', 'disputo' ) . '</div>';
			} else {
				$content = '<div class="alert alert-info">' . esc_html__( 'This is a private reply.', 'disputo' ) . '</div>' . $content;
			}
		}
...
$GLOBALS['bbp_private_replies'] = new Hovard_BBP_Private_Replies();
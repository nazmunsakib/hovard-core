...
class bbp_solved_topic {
	...
	public function add_topic_admin_link( $links, $topic_id ) {
		...
		if ((current_user_can( $this->capability )) || ($topic_author == get_current_user_id())) {
			if (isset($_GET['unsolve_topic'])) {
				...
				$links['solved'] = '<a href="' . bbp_get_topic_permalink() . '?solve_topic">' . esc_html__( 'Mark as solved', 'hovard-core' ) . '</a>';
			} else if (isset($_GET['solve_topic'])) {
				$this->solve_topic(bbp_get_topic_id());
				$links['unsolved'] = '<a href="' . bbp_get_topic_permalink() . '?unsolve_topic">' . esc_html__( 'Mark as unsolved', 'hovard-core' ) . '</a>';
			} else {
	...
	public function solve_topic( $topic_id = 0) {
		...
		if ((current_user_can( $this->capability )) || ($topic_author == get_current_user_id())) {
			...
			wp_die( esc_html__( 'You do not have the permission to do that!', 'hovard-core' ) );
		}
		...
		if ( bbp_is_reply_edit() && (current_user_can( $this->capability ))) {
			if ((!$has_best_answer ) || ($is_best_answer )) {
				...
				<p>
					...
					<label for="bbp_best_answer"><?php esc_html_e( "Set this reply as Best Answer.", 'hovard-core' ); ?></label>
				</p>
			<?php }
		}
	}
	...
	public function topic_checkbox() {
		...
		if ( bbp_is_topic_edit() && ((current_user_can( $this->capability )) || ($topic_author == get_current_user_id())) ) {  ?>
			<p>
				...
				<label for="bbp_solved_topic"><?php esc_html_e( "Set this post as solved.", 'hovard-core' ); ?></label>
			</p>
		<?php }
	...
	public function reply_content_filter( $content = '', $reply_id = 0 ) {
		...
		if( $this->is_best_answer( $reply_id )) {
			$content = '<div class="solved-topic-bar" id="best-answer"><span class="badge"><i class="icon_check"></i> ' . esc_html__( 'Best Answer', 'hovard-core' ) . '</span></div>' . $content;
		}
	...
	public function topic_content_filter( $content = '', $topic_id = 0 ) {
		...
		if( $this->is_solved( $topic_id ) ) {
			$resolved = '<span class="accepted-ans-mark"><i class="icon_check_alt"></i> ' . esc_html__( 'Resolved', 'hovard-core' ) . '</span>';
		}

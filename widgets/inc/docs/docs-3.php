        ...
        if ( $settings['is_custom_order'] == 'yes' && !empty($settings['docs']) ) {
            ...
            foreach ( $custom_docs as $doc_item ) {
                ...
                <li class="nav-item">
                    <a <?php echo $atts; ?> id="<?php echo $post_title_slug; ?>-tab" class="nav-link<?php echo esc_attr($active) ?>" data-toggle="tab">
                        <?php
                        echo get_the_post_thumbnail($doc_id, 'hovard_16x16');
            ...
            if ( $parent_docs ) :
                foreach ($parent_docs as $i => $doc) :
                    ...
                    ?>
                    <li class="nav-item">
                        <a <?php echo $href.$aria_controls; ?> id="doc3<?php echo $doc->post_name; ?>-tab" class="nav-link<?php echo esc_attr($active) ?>" data-toggle="tab">
                            <?php
                            echo get_the_post_thumbnail($doc->ID, 'hovard_16x16');
                            if ( $settings['is_tab_title_first_word'] == 'yes' ) {
                                echo wp_kses_post($doc_name[0]);
                            } else {
                                ...
                                <div class="topic_list_item">
                                    ...
                                    if ( !empty($settings['read_more']) ) : ?>
                                        <a class="text_btn dark_btn" href="#">
                                            <?php echo esc_html($settings['read_more']) ?> <i class="<?php hovardcore_arrow_left_right() ?>"></i>
                                        </a>
                                        <?php
                                    endif;
                                    ?>
